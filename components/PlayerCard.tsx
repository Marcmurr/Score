...
import type { PlayerState, PlayerKey, ScoreType } from '../types';
...
const PlayerCard: React.FC<PlayerCardProps> = ({ player, playerKey, turn, onNameChange, onScoreChange, onSecondaryMissionChange }) => {
  const totalPrimary = Object.values(player.scores).reduce((sum, turnScores) => sum + turnScores.primary, 0);
  const totalSecondary = Object.values(player.scores).reduce((sum, turnScores) => sum + turnScores.secondary, 0);
  ...
  return (
    <div className="bg-slate-800/50 backdrop-blur-sm border-2 border-slate-700 rounded-lg p-4 md:p-6 w-full flex flex-col gap-4 shadow-lg">
      <input
        type="text"
        value={player.name}
      ...
      <div className="grid grid-cols-2 gap-4 text-center">
        ...
        <div className="flex flex-col items-center gap-2 p-2 bg-slate-900/50 rounded-md">
          ...
          <div className="flex items-center gap-3">
            <IconButton onClick={() => onScoreChange(playerKey, 'commandPoints', -1)} ariaLabel="Decrease Command Points"><MinusIcon /></IconButton>
            <span className="font-orbitron text-3xl text-white w-12">{player.commandPoints}</span>
            <IconButton onClick={() => onScoreChange(playerKey, 'commandPoints', 1)} ariaLabel="Increase Command Points"><PlusIcon /></IconButton>
          </div>
        </div>
      ...
      <div className="flex flex-col items-center gap-2 p-3 bg-slate-900/50 rounded-md">
        ...
        <div className="flex items-center gap-3">
          <IconButton onClick={() => onScoreChange(playerKey, 'primaryScore', -1, turn)} ariaLabel="Decrease Primary Score by 1"><MinusIcon /></IconButton>
          <span className="font-orbitron text-3xl text-white w-12 text-center">{turnScores.primary}</span>
          <IconButton onClick={() => onScoreChange(playerKey, 'primaryScore', 1, turn)} ariaLabel="Increase Primary Score by 1"><PlusIcon /></IconButton>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center gap-2 p-3 bg-slate-900/50 rounded-md">
        ...
        <div className="flex items-center gap-3">
          <IconButton onClick={() => onScoreChange(playerKey, 'secondaryScore', -1, turn)} ariaLabel="Decrease Secondary Score by 1"><MinusIcon /></IconButton>
          <span className="font-orbitron text-3xl text-white w-12 text-center">{turnScores.secondary}</span>
          <IconButton onClick={() => onScoreChange(playerKey, 'secondaryScore', 1, turn)} ariaLabel="Increase Secondary Score by 1"><PlusIcon /></IconButton>
        </div>
      </div>
